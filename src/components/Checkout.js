  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick={(e)=>{handleNewAddress((cur)=>({...cur,isAddingNewAddress:false,value:""}))}}
        >
          Cancel
        </Button>
      </Stack>
              ...
              (addresses.all.map((add)=>{
                ...
                return (
                  <div className={classname} onClick={(e)=>{setAddresses({...addresses, selected:add["_id"]})}} key={add["_id"]}>
                    ...
                    <Button startIcon={<Delete />} id={add["_id"]} onClick={(e)=>{deleteAddress(token, e.target.id)}} >Delete</Button>
                  </div>
                )  
              }))               
               :(<Typography my="1rem">
